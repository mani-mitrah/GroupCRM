...
var base_url = 'https://crm.ontimegroup.com/api/livechat/';
var app_url = 'https://crm.ontimegroup.com/';
	    ...
	    success: function(res) {
			...
			for (var i = 0; i < res.length; i++){
			    ...
			    for (var key in obj){
			        ...
			        {
			        	customer_name = (attrValue !='')?attrValue:"Customer";
			        }

			        if(key==='customer_email')
			        {
			        	customer_email = (attrValue !='')?attrValue:"Customer Email";
			        }

			        if(key==='customer_mobile')
			        {
			        	customer_mobile = (attrValue !='')?attrValue:"Customer Mobile";
			        }
			    }
			    ...
			    let paused_conversations = $('<li id="con'+conversation_id+'" class="contacts-item friends"><a class="contacts-link" data-acid="'+ac_id+'" data-pgid="'+pg_id+'" data-agentid="'+agent_id+'" data-botid="'+bot_id+'" data-coversationid="'+conversation_id+'" data-created="'+created_at+'" data-customerid="'+customer_id+'" data-email="'+customer_email+'" data-fullname="'+customer_name+'" data-mobile="'+customer_mobile+'" data-updatedat="'+last_updated_at+'"  data-isclosed="0"   href="javascript:void(0);" onclick="javascript:expand(this);"><div class="timeline-panel"><div class="media mr-2 media-danger">'+customer_name.substring(0,2).toUpperCase()+'</div><div class="media-body"><h5 class="mb-1">'+customer_name+'</h5><small class="d-block"><br/>'+customer_mobile+'<br />'+moment.utc(last_updated_at).local().fromNow()+'</small></div><div class="dropdown"><div id="concount'+conversation_id+'" style="display:none;" class="badge badge-rounded badge-primary ml-1">0</div></div></div></a></li>');
			    paused_conversations.appendTo("#ongoing_conversations");
	    ...
	    success: function(res) {
			...
			for (var i = 0; i < res.length; i++)
			{
			    ...
			    if(type==='session_reset') {
			    	...
			    	let user_conversations = $('<div class="message '+message_side+'"><div class="message-wrapper"><div class="message-content"><h6 class="text-dark">'+belongs_to+'</h6><span>'+message+'</span></div></div><div class="message-options"><div class="avatar avatar-sm"><img alt="" src="'+app_url+'/global/images/'+user_icon+'"></div><span class="message-date">'+moment.utc(timestamp).local().fromNow()+'</span></div></div>');
					user_conversations.appendTo("#user_conversation");
		...
		if(conversationId===expanded_conversation_id)
		{
			...
			if(payload_type==='text' || payload_type==='quick_reply' || payload_type==='session_reset' )
			{
				if(payload_type==='session_reset') {
			    	...
			    	let user_conversations = $('<div class="message '+message_side+'"><div class="message-wrapper"><div class="message-content"><h6 class="text-dark">'+belongs_to+'</h6><span>'+message_text+'</span></div></div><div class="message-options"><div class="avatar avatar-sm"><img alt="" src="'+app_url+'/global/images/'+user_icon+'"></div><span class="message-date">'+moment.utc(sent_on).local().fromNow()+'</span></div></div>');
					user_conversations.appendTo("#user_conversation");
				}
				$("#chat-finished").get(0).scrollIntoView();
		   	}
...
function notify_ongoing(event)
{
	//recent.onmessage = function(event) {
		...
		if(new_count > 0)
		{
            ...
            var audio = new Audio(app_url+"global/sound/notification.mp3"); 
            audio.play(); 
		}
	//};
}
	...
	$.ajax({
	    ...
	    success: function(res) {
			...
			for (var i = 0; i < res.length; i++){
			    ...
			    let all_user_data = $('<li id="con'+conversation_id+'" class="contacts-item friends"><a class="contacts-link" data-acid="'+ac_id+'" data-agentid="'+agent_id+'" data-botid="'+bot_id+'" data-coversationid="'+conversation_id+'"  data-customerid="'+customer_id+'" data-email="'+email_address+'" data-fullname="'+full_name+'" data-mobile="'+mobile_number+'" data-updatedat="'+last_updated_at+'" data-isclosed="1"   href="javascript:void(0);" onclick="javascript:expand(this);"><div class="timeline-panel"><div class="media mr-2 media-danger">'+full_name.substring(0,2).toUpperCase()+'</div><div class="media-body"><h5 class="mb-1">'+full_name+'</h5><small class="d-block">'+mobile_number+'<br />'+moment.utc(last_updated_at).local().fromNow()+'</small></div><div class="dropdown"><div id="concount'+conversation_id+'" style="display:none;" class="badge badge-rounded badge-primary ml-1">0</div></div></div></a></li>');
			    all_user_data.appendTo("#all_conversations");
