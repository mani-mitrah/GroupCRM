...
class Conversation extends REST_Controller
{
    ...
    public function pause_post() {
        ...
        log_message('error','customer_name:'.$name);
        log_message('error','customer_email:'.$email);
        log_message('error','customer_mobile:'.$mobile);
        ...
        if($status_code == 200) {
            $create_temp_record = $this->mcommon->common_edit('lc_conversations',array('customer_name'=>$name,'customer_email'=>$email,'customer_mobile'=>$mobile), array('botId'=>$bot_id,'userId'=>$user_id,'thread_id'=>$conv_id));
    ...
    public function end_chat_get($bot_id, $user_id, $pg_id) {
        ...
        if($status_code == 200) 
        {
            ...
            $completed = $this->mcommon->common_edit('lc_conversations',array('is_completed'=>1),array('pg_id'=>$pg_id));
            if($completed) {
                return $this->response("OK", 200);    
            } else {
    ...
    public function agent_takeover_get()
    {
        ...
        $insert_agent_chat = $this->mcommon->common_insert('lc_agent_chats',array('agent_id'=>$agent_id,'conversation_id'=>$pg_id));
        if($insert_agent_chat >= 1)
        {
            $update_conversation = $this->mcommon->common_edit('lc_conversations',array('is_assigned'=>1),array('pg_id'=>$pg_id));
    ...
    public function check_post()
    {
        ...
        $check = $this->mcommon->specific_record_counts('lc_conversations',array('pg_id'=>$pg_id));
        if($check > 0)
        {
            ...
            $update = $this->mcommon->common_edit('lc_conversations',$update_array,array('pg_id'=>$pg_id));
            if($update)
            {
                $is_assigned = $this->mcommon->specific_row_value('lc_conversations',array('pg_id'=>$pg_id),'is_assigned');
        ...
        else
        {
            ...
            $insert = $this->mcommon->common_insert('lc_conversations',$insert_array);
            if($insert >= 1)
            {
    ...
    public function welcome_message_get($company_id) {
        ...
        $message = $this->mcommon->specific_row_value("lc_chatbot_messages", array("company_id"=>$company_id), "welcome_message");
        if(empty($message)) {
