...
class Master_model extends CI_Model
{
    ...
    {
        $this->db->select("crm_timeslots.*,CONCAT(users.first_name,' ',users.last_name) as creator");
    ...
    {
        $this->db->select("crm_timeslot_exception.*,CONCAT(users.first_name,' ',users.last_name) as creator");
    ...
    public function get_user_primary_group($id)
    {
        $this->db->select("groups.group_id");
        $this->db->from("groups");
        $this->db->join("group_members", "group_members.group_id=groups.group_id and group_members.user_id = " . $id, "left");
        $this->db->where('group_members.is_primary_group_id', 1);
        $query = $this->db->get();
    ...
    public function get_report_groups($id)
    {
        $this->db->select("groups.*,IF(users_group_report.ug_user_id=" . $id . ",1,0) as is_selected");
        $this->db->from("groups");
        $this->db->where("groups.status=", 1);
        $this->db->join("users_group_report", "users_group_report.ug_group_id=groups.group_id and users_group_report.ug_user_id = " . $id, "left");
        $query = $this->db->get();
    ...
    {
        $this->db->select("*");
        $this->db->from('ontime_category_services_ as l');
    ...
    public function lead_categories($id = "")
    {
        if ($id == "") {
            ...
            $this->db->join('ontime_category_services_ as c', 'l.category_id=c.category_id and c.is_active = 1');
    ...
    public function package_services()
    {
        ...
        $this->db->join('ontime_category_services_ as s', 'l.service_id=s.service_id');
        $query = $this->db->get();
        return $query->result_array();
    }
    ...
    public function workflows()
    {
        ...
        $this->db->join('ontime_category_services_ as ls', 'ls.service_id=lw.parent_service_id');
        $query = $this->db->get();
        return $query->result_array();
    }
    ...
    public function workflow_entries($workflow_id = '')
    {
        ...
        $this->db->join('ontime_category_services_ as ls', 'lwe.target_service_id=ls.service_id');
        if ($workflow_id != '') {
    ...
    {
        $this->db->select("calendar_group.*,CONCAT(users.first_name,' ',users.last_name) as creator,count(DISTINCT calendar_members.calendar_member_id) as members_count");
        $this->db->from("calendar_group");
        $this->db->join("calendar_members", "calendar_members.calendar_id=calendar_group.calendar_id", "left");
        $this->db->join("users", "users.user_id=calendar_group.created_by");
        $this->db->where("calendar_group.status=", 1);
        $this->db->group_by("calendar_group.calendar_id", "asc");
        $this->db->order_by("calendar_group.calendar_name");
        // $this->db->where("groups.status =","1");
        $query = $this->db->get();
        // print_r($this->db->last_query());
        // exit();
    ...
    public function get_calendar_data($id)
    {
        $this->db->select("calendar_group.*,IF(calendar_members.user_id=" . $id . ",1,0) as is_selected");
        $this->db->from("calendar_group");
        $this->db->where("calendar_group.status=", 1);
        $this->db->join("calendar_members", "calendar_members.calendar_id=calendar_group.calendar_id and calendar_members.user_id = " . $id, "left");
        $query = $this->db->get();

        return $query->result_array();
    }

    public function calendar_websites($id = "")
    {
        if ($id == "") {
            $this->db->select("*")->from("ontime_websites")->where("is_active", 1);
            $result = $this->db->get()->result_array();
            ...
            $this->db->select("ontime_websites.*,if(calendar_website.calendar_web_id IS NOT NULL,'1','0') as is_selected")->where("ontime_websites.is_active", 1)->from("ontime_websites");
            $this->db->join("calendar_website", "calendar_website.website_id=ontime_websites.id and calendar_website.calendar_id=" . $id, "left");
            // print_r($this->db->last_query());
            $result = $this->db->get()->result_array();
            return $result;
        }
    ...
    public function get_calendarnotificationsettings($id)
    {
        $this->db->select("*")->from("calendar_settings")->where("calendar_id", $id);
        $result = $this->db->get()->result_array();
        return isset($result[0]) ? $result[0] : array();
    }

    public function get_calendarnotificationsettings_list()
    {
        $this->db->select("*")->from("calendar_settings");
        $result = $this->db->get()->result_array();
        return isset($result[0]) ? $result[0] : array();
    }

    public function get_calendar_branches()
    {
    ...
    public function get_calendar_branch($id)
    {
        $this->db->select("ontime_branches.*,IF(calendar_members.user_id=" . $id . ",1,0) as is_selected");
        $this->db->from("ontime_branches");
        $this->db->where("ontime_branches.is_active=", 1);
        $this->db->join("calendar_members", "calendar_members.calendar_branch=ontime_branches.branch_code and calendar_members.user_id = " . $id, "left");
        $query = $this->db->get();
        return $query->result_array();
    }

    public function get_tl_by_group_id($group_id,$id)
    {
        $this->db->select("users.first_name, users.last_name,users.user_id");
