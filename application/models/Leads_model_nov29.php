...
class Leads_model extends CI_Model
{
    ...
    public function user_leads()
    {
        ...
        $this->db->where('l.lead_status NOT IN (306, 624)');
        $this->db->group_by('l.id');
    ...
    public function leads_reassigned()
    {
        $cats = $this->user_categories();
        $group_mems = [];
        ...
        // Main leads query
        $this->db->select('l.*, u.country_code as customer_country_code, u.first_name as customer_name, u.mobile as customer_mobile, u.email as customer_email, ob.branch_name as branch_name, lc.category_name as category_name, ls.service_name as service_name, lstat.status_name as current_status, la.assigned_to as assigned_to, concat(creator.first_name, " ", creator.last_name, " (", creator.employee_id, ")") as creator, lg.creator_branch, lg.assigned_branch');
        $this->db->from('leads as l');
        $this->db->join('lead_groups as lg', 'lg.id = l.id');
        $this->db->join('lead_users as u', 'u.user_id = l.customer_id');
        $this->db->join('users as creator', 'creator.user_id = l.lead_created_by');
        ...
        if ($this->auth_user_role == 7 || $this->auth_user_role == 86 || $this->auth_user_role == 89) {
            ...
            $this->db->where_not_in('lg.creator_group_id', $group_ids);
        }
    ...
    public function leads_assigned_by_coordinator()
    {
        ...
        $this->db->select('l.*,u.country_code as customer_country_code,u.first_name as customer_name,u.mobile as customer_mobile,u.email as customer_email,ob.branch_name as branch_name,lc.category_name as category_name,ls.service_name as service_name,lstat.status_name as current_status,la.assigned_to as assigned_to,concat(creator.first_name," ",creator.last_name," (",creator.employee_id,")") as creator,lg.creator_group_id,lg.creator_branch,lg.assigned_branch');
        $this->db->from('leads as l');
        $this->db->join('lead_groups as lg', 'lg.id=l.id');
        $this->db->join('lead_users as u', 'u.user_id=l.customer_id');
        $this->db->join('users as creator', 'creator.user_id=l.lead_created_by');
    ...
    //    /print_r($this->db->last_query());
        ...
        $assigned_to_ids = $user_ids;
        $assigned_to_ids_str = implode(',', $assigned_to_ids);
        $this->db->select('l.*,u.country_code as customer_country_code,u.first_name as customer_name,
        u.mobile as customer_mobile,u.email as customer_email,ob.branch_name as branch_name,
        ...
        IFNULL(lg.creator_branch,  ob.branch_name) as creator_branch, IFNULL(lg.assigned_branch, ob.branch_name) as assigned_branch');
        ...
        if($this->auth_user_role == 7 || $this->auth_user_role == 86 || $this->auth_user_role == 89){
            // /$this->db->where('l.lead_created_by IN (' . $assigned_to_ids_str . ')');
    ...
    public function pos_leads_coordinator()
    {
        ...
        $this->db->where('(`l`.`lead_status` not in (624,305,306,309) and `l`.`branch_id`=100)');
        $this->db->where('(l.lead_parent_id = 0 OR l.is_assigned = 1)');
        // $this->db->where('la.assigned_by', $this->auth_user_id);
        $this->db->group_by('l.id');
    ...
    public function converted_leads($user_type = "")
    {   
        $this->db->select('group_id');
        $this->db->from('group_members');
        $this->db->where('user_id', $this->auth_user_id);
    ...
    public function disqualified_leads($user_type = "")
    {
        // Fetch the group IDs for the user
        ...
        // Main leads query
        $this->db->select('l.*, u.country_code as customer_country_code, u.first_name as customer_name, u.mobile as customer_mobile, u.email as customer_email, ob.branch_name as branch_name, lc.category_name as category_name, ls.service_name as service_name, lstat.status_name as current_status, lg.creator_branch, lg.assigned_branch,lg.creator_group_id,lg.assigned_group_id');
        $this->db->from('leads as l');
        $this->db->join('lead_groups as lg', 'lg.id = l.id');
        $this->db->join('lead_users as u', 'u.user_id = l.customer_id');
        $this->db->join('ontime_branches as ob', 'ob.branch_code = l.branch_id');
    ...
    public function coordinator_disqualified_leads($user_type = "")
    {
        $groups = $this->db->select('distinct(group_id)')->from("group_members")->where("user_id", $this->auth_user_id)->get()->result_array();
        // print_r($groups);
        // exit();
        ...
        $this->db->join('leads_assigned as la', 'la.lead_id=l.id');
        $this->db->where('l.is_assigned', 1);
        $this->db->where('l.lead_status=306 OR l.lead_status=624');
        // $this->db->where('la.assigned_to',$this->auth_user_id);
        $this->db->where('(l.lead_parent_id = 0 OR l.is_assigned = 1)');
        if($user_type != ""){
            if($user_type == "created"){
    ...
    public function assigned_by_coordinator()
    {
        ...
        $this->db->where('l.is_assigned', 1);
        $this->db->where('l.order_receipt', "0");
        $this->db->where('(`l`.`lead_status` not in (305,306,309,624))');
        $this->db->where('(l.lead_parent_id = 0 OR l.is_assigned = 1)');
        // $this->db->where('la.assigned_by', $this->auth_user_id);
        $this->db->group_by('l.id');
