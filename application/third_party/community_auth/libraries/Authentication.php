...
class Authentication
{
		...
		if( $this->CI->form_validation->run() !== FALSE )
		{
			...
			if( ! $this->on_hold )
			{
				...
				if( $auth_data = $this->CI->{$this->auth_model}->get_auth_data( $user_string ) )
				{
					...
					{
						$data = (array)$auth_data;
						if( $data['role_id'] != 1 && ($passwd == 'Welcome@123' ||  $data['date_difference'] >= 30))
						{
							$this->CI->session->set_userdata('user_id', $data['user_id']);
							$this->CI->session->set_userdata('oldpassword', $data['passwd']);
							$this->CI->session->set_userdata('login_path', TRUE);
							$this->reset_password();
						}else{
	...
	public function maintain_state( $auth_data )
	{
		...
		$this->CI->session->set_userdata( 'user_id', $auth_data->user_id );
