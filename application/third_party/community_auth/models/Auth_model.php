...
class Auth_model extends MY_Model {
	...
	public function get_auth_data( $user_string )
	{
		...
		{
			$row = $query->row_array();
			$date = ($row['passwd_modified_at'] != '') ? $row['passwd_modified_at'] : $row['created_at'];
			$row['date_difference'] = $this->passwordUpdatedDate($date);
	...
	public function check_login_status( $user_id, $login_time )
	{
		...
		$this->db->select( $selected_columns )
			...
			//->join( $this->db_table('company_users_table') . ' cu', 'cu.user_id = u.user_id' )
			//->join( $this->db_table('company_table') . ' c', 'c.id = cu.company_id' )
	...
	public function logout($user_id, $session_id)
	{
		...
		// Get the current time (logout time)
		$logout_time = date('Y-m-d H:i:s');  // or use Carbon if you're using it

		// Fetch the session record by user_id and session_id
		$session = $this->db->select('login_time')
							->where('user_id', $user_id)
							->where('id', $session_id)
							->get($this->db_table('auth_sessions_table'))
							->row();

		// If session exists and we have a login time
		if ($session && $session->login_time) {
			...
			$duration = $login_time->diff($logout_time_obj);
			$duration_seconds = ($duration->h * 3600) + ($duration->i * 60) + $duration->s;

			// Update the session record with logout time and duration
			$this->db->where('user_id', $user_id)
					->where('id', $session_id)
					->update($this->db_table('auth_sessions_table'), [
						'logout_time' => $logout_time,
						'duration'    => $duration_seconds
					]);
		} else {
			// If no session found or missing login_time
			// You could handle this error case as per your requirement
			log_message('error', 'Session not found or login time missing.');
		}
	}
	...
	/**
	 * last password updated date
	 * @param $date
	 * @return int
	 * 
	 */
	public function passwordUpdatedDate($date)
	{
		$today = date('Y-m-d H:i:s');
		$differenceInSeconds = strtotime($today) - strtotime($date);
