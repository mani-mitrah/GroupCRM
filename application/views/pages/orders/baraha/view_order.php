...
<div class="row page-titles mx-0">
    ...
    <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
        <a class="btn btn-primary waves-effect waves-light float-right"
            href="<?php echo base_url(); ?>orders/baraha/">
            <i class="fa fa-back mr-2"></i> Back to orders
        </a>
    </div>
</div>
<div class="row">
    <div class="col-lg-12">
        <div class="card" id="order_details_block">
            <div class="card-header">
              ...
              ?>
                <h5 class="card-title">ORDER DETAILS <span class="badge badge-primary text-white"><?php echo 'BOOKING ID: B'.$order_details['order_id'].'-'.$order_details['item_id']; ?></span>&nbsp;&nbsp;<span class="badge badge-<?php echo $badge; ?>"><?php echo 'STATUS: '.$order_details['status_name']; ?></span></h5>
            </div>
            <div class="card-body">
                ...
                <div class="mt-2">
                  <h5><strong>MEDICAL REFERENCE NUMBER</strong></h5>
                  <form accept="<?php echo base_url();?>orders/baraha/view?code=<?php echo $_GET['code']; ?>" method="post" id="mrn_block">
                    ...
                    <label><br /><br /></label>
                    <input type="hidden" name="item_id" value="<?php echo $_GET['code']; ?>" id="item_id">
                    <input type="submit" class="btn btn-primary" name="submit_mrn" id="mrn_update">
                  </form>
                  <?php
                  if($order_details['med_number']!='0')
                  {
                    ?>
                    <h6><strong>Medical Reference Number:</strong><br /><?php echo $order_details['med_number']; ?><br /></h6>
                    <?php

                  }

                  if($order_details['ref2']!='')
                  {
                    ?>
                    <h6><strong>Ref2:</strong><br /><?php echo $order_details['ref2']; ?><br /></h6>
                    <?php
                ...
                {
                ?>
                <div class="appointment_block mt-2">
                  <h5><strong>APPOINTMENT INFORMATION</strong></h5><br >
                  <strong>Appointment Date</strong>: <?php echo $appointments['appointment_date']; ?><br /><br />
                  <strong>Appointment Time</strong>: <?php echo $appointments['appointment_time']; ?>
                </div>
                <?php
                }
                ?>
...
<script type="text/javascript">
    ...
    $.ajax({
      url: "<?php echo base_url(); ?>api/v1/order/item?item_id=<?php echo $_GET['code']; ?>",
      type: 'GET',
    ...
    $.ajax({
      url: "<?php echo base_url(); ?>api/v1/order/attachments?item_id=<?php echo $_GET['code']; ?>",
      type: 'GET',
      dataType: 'json', // added data type
      success: function(res) {
          ...
          for (var i = 0; i < res.length; i++) {
            let attachment_url = res[i].attachment;
            let attachment_link = attachment_url.replace(/["']/g, "");
            let attachment_data = $('<tr><td><a target="_blank" href="'+attachment_link+'">View Attachment</a></td></tr>');
