...
class Aladheed extends MY_Controller
{
	...
	public function index()
	{
		if ($this->verify_min_level(1)) 
		{
			...
			{
		    	$view_data['enquiries'] = $this->app_model->enquiries(507);
		    }
		    else
		    {
		    	$view_data['enquiries'] = $this->app_model->csa_enquiries(507);	
		    }

			$data = array(
				'page_title' => 'Enquiries',
				'title' => 'Enquiries',
				'content' => $this->load->view('pages/enquiries/aladheed/list', $view_data, TRUE),
			);
	...
	public function assign()
	{
		if ($this->verify_min_level(1)) 
		{
			...
			if($this->auth_user_role==1 || $this->auth_user_role==6)
			{
		    	$view_data['enquiries'] = $this->app_model->unassigned_enquiries(507);
			...
			$view_data['website_details'] = $this->mcommon->specific_row('ontime_websites', array('id'=>'507'));
				...
				$data = array(
					...
					'content' => $this->load->view('pages/enquiries/aladheed/unassigned_list', $view_data, TRUE),
				);
				$this->load->view('template/full_template', $data);
			}
		} 
	...
	public function view()
	{
		if ($this->verify_min_level(1)) 
		{
			...
			$view_page_url = 'enquiries/aladheed/view/?id=';
			$enquiry_type=1;
