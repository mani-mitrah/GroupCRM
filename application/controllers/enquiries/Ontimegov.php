...
class Ontimegov extends MY_Controller
{
	...
	public function index()
	{
		if ($this->verify_min_level(1)) 
		{
			...
			{
		    	$view_data['enquiries'] = $this->app_model->enquiries(502);
		    }
		    else
		    {
		    	$view_data['enquiries'] = $this->app_model->csa_enquiries(502);	
		    }

			$data = array(
				'page_title' => 'Enquiries',
				'title' => 'Enquiries',
				'content' => $this->load->view('pages/enquiries/ontimegov/list', $view_data, TRUE),
			);
	...
	public function assign()
	{
		if ($this->verify_min_level(1)) 
		{
			...
			if($this->auth_user_role==1 || $this->auth_user_role==6)
			{
		    	$view_data['enquiries'] = $this->app_model->unassigned_enquiries(502);
		    	log_message('error',$this->db->last_query());
		    	$view_data['csas'] = $this->user_model->website_csas(502);
				$data = array(
					'page_title' => 'Un-assigned Enquiries',
					'title' => 'Un-assigned Enquiries',
					'content' => $this->load->view('pages/enquiries/ontimegov/unassigned_list', $view_data, TRUE),
				);
				$this->load->view('template/full_template', $data);
			}
		} 
	...
	public function view()
	{
		if ($this->verify_min_level(1)) 
		{
			...
			$view_page_url = 'enquiries/ontimegov/view/?id=';
			$enquiry_type=1;
