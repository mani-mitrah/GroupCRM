...
class All extends MY_Controller
{
	...
	public function index()
	{
		...
		if ($this->verify_min_level(1)) {
			if ($this->auth_user_role == 1 || $this->auth_user_role > 5) {
				$view_data['enquiries'] = $this->app_model->csa_enquiries();
			} else {
				$view_data['enquiries'] = $this->app_model->enquiries();
			}
			// print_r(json_encode($view_data["enquiries"]));
			// exit();
			$data = array(
				'page_title' => 'Enquiries',
				'title' => 'Enquiries',
				'content' => $this->load->view('pages/enquiries/all/index', $view_data, TRUE),
			);
	...
	public function assign()
	{
		if ($this->verify_min_level(1)) {
			...
			if ($this->auth_user_role == 1 || $this->auth_user_role > 5) {
				$view_data['enquiries'] = $this->app_model->unassigned_enquiries();
				log_message('error', $this->db->last_query());
				$view_data['csas'] = $this->user_model->website_csas();
				$data = array(
					'page_title' => 'Un-assigned Enquiries',
					'title' => 'Un-assigned Enquiries',
					'content' => $this->load->view('pages/enquiries/all/unassigned_list', $view_data, TRUE),
				);
	...
	public function view()
	{
		if ($this->verify_min_level(1)) {
			...
			// print_r($view_data['enquiry_details']["website_id"]);
			$view_data['website_details'] = $this->mcommon->specific_row('ontime_websites', array('id' => $view_data['enquiry_details']["website_id"]));
			...
			$view_page_url = 'enquiries/all/view/?id=';
			$enquiry_type = 1;
			...
			if (isset($_POST['action_email'])) {
				...
				// echo "</pre>";
				// exit();
				action_send_email($enquiry_id, $customer_email, $agent_email, $email_subject, $email_message, $email_remarks, $contactable_date, $logged_in_user_id, $view_page_url, $website_id);
			}
			...
			$data = array(
				'page_title' => 'Enquiries',
				'title' => 'Enquiries',
				'content' => $this->load->view('pages/enquiries/all/view_enquiry.php', $view_data, TRUE),
			);
			$this->load->view('template/base_template_v2', $data);
		} else {
			redirect('login');
