...
class Lead extends MY_Controller
{
        ...
        if ($this->verify_min_level(1)) {
            if (isset($_POST['submit'])) {
                ...
                if ($payment_type == "online") {
                    ...
                    $update = $this->mcommon->common_edit("leads", array("lead_status" => 312, 
                    "pos_pmt_number" => $raw_salesorder, "pos_cust_key" =>  $pos_cust_key), array("id" => $lead_id));
    ...
    public function lab_new()
    {
        if ($this->verify_min_level(1)) {
            ...
            $view_data['services'] = $this->leads_model->lead_services();
            $view_data['categories'] = $this->mcommon->specific_fields_records_all('ontime_categories', array('is_active' => 1));
            $view_data['packages'] = $this->mcommon->specific_fields_records_all('lead_packages', array('is_active' => 1, "package_branch" => 109));
            $data = array(
        ...
        if ($this->verify_min_level(1)) {
            ...
            if (isset($_POST['submit'])) {
                try {
                    ...
                    $user_id = $this->customer_handle($data["customer_name"], $_POST["lead_contact"], $_POST["customer_email"], $_POST["lead_country_code"]);
                    $created_group_id = $this->mcommon->specific_row_value('group_members', array('user_id' => $this->auth_user_id, 'is_primary_group_id' => 1), 'group_id');
    ...
    public function view($lead_id)
    {
        if ($this->verify_min_level(1)) {
            ...
            $view_data["payment_histories"] = $this->db->select("*")->from("lead_action_log")->join("leads","leads.id=lead_action_log.lead_id")->where("(leads.id=".$lead_id.") or leads.lead_parent_id=".$lead_id.")")->where_in("lead_action_log.action_id",[307,308,310,311,312,320,321,322,323,325,326,327,328,329])->get()->result_array();
            // print_r($view_data["payment_histories"]);
            // exit();
    ...
    public function customer_handle($lead_name, $lead_contact, $lead_email, $lead_country_code)
    {
    ...
    public function action_addpayment($lead_id)
    {
        if ($this->verify_min_level(1)) {
            ...
            if (isset($_POST['action_payment'])) {
                ...
                $this->mcommon->common_edit("leads", array("additional_govt_fee" => $this->input->post('amount_payment')), array("id" => $sub_lead_id));
                if ($service_name != null) {
                    // print_r($service_name);
                    // print_r($bot_id);
                    // exit();
                    ...
                    if ($order_id != '') {
                        ...
                        if ($insert_log > 0) {
                            ...
                            foreach ($users as $user) {
                                if($user["email"] == "fatma@ontimebiz.com"){
                                    continue;
                                }
        ...
        if ($this->verify_min_level(1)) {
            if (isset($_POST['action_close'])) {
                ...
                if ($close_remarks != '') {
                    ...
                    $insert_log = $this->mcommon->common_insert('lead_action_log', $log_insert_array);
                    if ($insert_log > 0) {
                        $update_lead_array = array('lead_status' => 306);
                        $update_lead = $this->mcommon->common_edit('leads', $update_lead_array, array('id' => $lead_id));
                        $this->session->set_flashdata('alert_complete', 'success');
    ...
    public function fetch_search_summary()
    {
        $search = $_GET["search"];
        $userId = $this->db->from('users')->where('user_id', $this->auth_user_id)->select('users.employee_id')->get()->row_array();
        $curl = curl_init();
        curl_setopt_array(
            $curl,
            array(
                CURLOPT_URL => 'https://ontimesmartpos.net/api/ApiPos/FetchCustomerForQuo?searchText='.$search.'&username='.$userId['employee_id'],
                // CURLOPT_URL => 'http://94.200.55.118:8096/api/ApiPos/FetchCustomerForQuo?searchText='.$search.'&username='.$userId['employee_id'],
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 0,
