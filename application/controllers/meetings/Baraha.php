...
class Baraha extends MY_Controller
{
	...
	public function index()
	{
		if ($this->verify_min_level(1)) 
		{
			...
			{
		    	$view_data['enquiries'] = $this->app_model->meetings(501);
		    }
		    else
		    {
		    	$view_data['enquiries'] = $this->app_model->csa_meetings(501);
			...
			$data = array(
				'page_title' => 'Meetings',
				'title' => 'Meetings',
				'content' => $this->load->view('pages/meetings/baraha/list', $view_data, TRUE),
			);
	...
	public function assign()
	{
		if ($this->verify_min_level(1)) 
		{
			...
			if($this->auth_user_role==1 || $this->auth_user_role==6)
			{
		    	$view_data['enquiries'] = $this->app_model->unassigned_meetings(501);
		    	log_message('error',$this->db->last_query());
		    	$view_data['csas'] = $this->user_model->website_csas(501);
				$data = array(
					'page_title' => 'Un-assigned Meetings',
					'title' => 'Un-assigned Meetings',
					'content' => $this->load->view('pages/meetings/baraha/unassigned_list', $view_data, TRUE),
				);
				$this->load->view('template/full_template', $data);
			}
		} 
	...
	public function view()
	{
		if ($this->verify_min_level(1)) 
		{
			...
			$view_page_url = 'meetings/baraha/view/?id=';
			$enquiry_type=3;
