...
class Aladheed extends MY_Controller
{
	...
	public function index()
	{
		...
		{
			$view_data['website_details'] = $this->mcommon->specific_row('ontime_websites', array('id'=>'507'));
			if($this->auth_user_role==1 || $this->auth_user_role==6)
			{
		    	//$view_data['enquiries'] = $this->mcommon->specific_fields_records_all('enquiry',array('website_id'=>507, 'enquiry'=>2));
		    	$view_data['enquiries'] = $this->app_model->complaints(507);
			}
		    else
		    {
		    	$view_data['enquiries'] = $this->app_model->csa_complaints(507);	
		    }

			$data = array(
				'page_title' => 'Complaints',
				'title' => 'Complaints',
				'content' => $this->load->view('pages/complaints/aladheed/list', $view_data, TRUE),
			);
	...
	public function assign()
	{
		if ($this->verify_min_level(1)) 
		{
			...
			if($this->auth_user_role==1 || $this->auth_user_role==6)
			{
		    	$view_data['enquiries'] = $this->app_model->unassigned_complaints(507);
		    	log_message('error',$this->db->last_query());
		    	$view_data['csas'] = $this->user_model->website_csas(507);
				$data = array(
					'page_title' => 'Un-assigned Complaints',
					'title' => 'Un-assigned Complaints',
					'content' => $this->load->view('pages/complaints/aladheed/unassigned_list', $view_data, TRUE),
				);
				$this->load->view('template/full_template', $data);
			}
		} 
	...
	public function view()
	{
		if ($this->verify_min_level(1)) 
		{
			...
			$view_page_url = 'complaints/aladheed/view/?id=';
			$enquiry_type=2;
			...
			if(isset($_POST['action_email']))
			{
				...
				action_email($enquiry_type,$enquiry_id,$from_email,$agent_email,$email_subject,$email_message,$email_remarks,$contactable_date,$logged_in_user_id,$view_page_url,507);
			}
