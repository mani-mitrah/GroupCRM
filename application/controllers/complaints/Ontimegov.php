...
class Ontimegov extends MY_Controller
{
	...
	public function index()
	{
		if ($this->verify_min_level(1)) 
		{
			...
			{
		    	//$view_data['enquiries'] = $this->mcommon->specific_fields_records_all('enquiry',array('website_id'=>502, 'enquiry'=>2));
		    	$view_data['enquiries'] = $this->app_model->complaints(502);
			}
		    else
		    {
		    	$view_data['enquiries'] = $this->app_model->csa_complaints(502);	
		    }

			$data = array(
				'page_title' => 'Complaints',
				'title' => 'Complaints',
				'content' => $this->load->view('pages/complaints/ontimegov/list', $view_data, TRUE),
			);
	...
	public function assign()
	{
		if ($this->verify_min_level(1)) 
		{
			...
			if($this->auth_user_role==1 || $this->auth_user_role==6)
			{
		    	$view_data['enquiries'] = $this->app_model->unassigned_complaints(502);
		    	$view_data['website_details'] = $this->mcommon->specific_row('ontime_websites', array('id'=>'502'));
				...
				$data = array(
					...
					'content' => $this->load->view('pages/complaints/ontimegov/unassigned_list', $view_data, TRUE),
				);
				$this->load->view('template/full_template', $data);
			}
		} 
	...
	public function view()
	{
		if ($this->verify_min_level(1)) 
		{
			...
			$view_page_url = 'complaints/ontimegov/view/?id=';
			$enquiry_type=2;
