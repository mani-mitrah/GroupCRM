...
class Complaints extends MY_Controller
{
		...
		public function index()
	{
		if( $this->verify_min_level(1))
		{
			...
			$view_data['complaints']=$this->complents();
			$view_data['assigned_complaints'] = $this->assigned_complaints();
			$view_data['company_user'] = $this->company_user(); 
			$data = array(
				'page_title' => 'Complaints',
				'title' => 'Complaints',
				'content' => $this->load->view('pages/complaints/view', $view_data, TRUE),
			);
	...
	public function complents(){
		$this->db->select("*");
		$this->db->from("enquiry");
		$this->db->where("enquiry",2);
		$query=$this->db->get();
		return $query->result();
	}
	 public function assigned_complaints(){
		$this->db->select("*");
		$this->db->from("assigned_complaints as e");
		$result=$this->db->get()->result();
	...
	public function assign_user(){
		...
		$insert=$this->db->insert("assigned_complaints",$insert_array);
		if($insert){
...
public function view_enquiry($id){
	...
	$data = array(
		...
		'title' => 'View Enquiry',
		'content' => $this->load->view('pages/complaints/details', $view_data, TRUE),
	);
	$this->load->view('template/base_template', $data);
}
