...
class Common_model extends CI_Model
{
	...
	public function specific_fields_records_all($table, $constraint_array='',$get_field_array='',$group_by='',$order_by_column='',$order_by='')
	{
	...
	*/
	...
	public function chat_snippets($table, $constraint_array='',$start='',$end='',$get_field_array='',$group_by='',$order_by_column='',$order_by='')
	{
		if(!empty($get_field_array))
		{
			$this->db->select($get_field_array);
		...
		if($start!='' && $end!='')
		{
			$this->db->where('`sort`>='.$start.' AND `sort` <='.$end);
		}
