...
class Verify extends REST_Controller
{
    ...
    public function verify_agent_get()
    {
        $cart_id = $this->get('cart_id');
        $agent_code = $this->get('agent_code');

        if ($cart_id == '' || $agent_code == '') {
            $error_array = array('status' => 'error', 'message' => 'Parameters Missing');
            $this->response(array($error_array), 400);
        } else {
            $branch_id = $this->mcommon->specific_row_value('bag', array('bag_id' => $cart_id), 'branch_id');
            $exist_agent = $this->mcommon->specific_row_value('em_branches', array('id' => $branch_id), 'agent_code');

            if ($agent_code == $exist_agent) {
                $error_array = array('status' => 'success', 'message' => 'Agent Code Verified');
                $this->response(array($error_array), 200);
            } else {
                $error_array = array('status' => 'error', 'message' => 'Agent Mismatch');
                $this->response(array($error_array), 403);
            }
