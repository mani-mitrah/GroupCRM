...
class App_model extends CI_Model
{
    ...
    public function get_last_conversation($timestamp='')
    {
        ...
        $this->db->from('temp_conversations');
        if($timestamp!='')
        {
    ...
    {
        $this->db->select('*');
        $this->db->from('agent_chats as ac');
        $this->db->join('conversations as c','ac.conversation_id=c.pg_id');
    ...
    function get_random_quote()
    {
        ...
        $this->db->limit(1);
        $query = $this->db->get('random_quotes');
